document.addEventListener("DOMContentLoaded", function () {
  ...
  class Note {
    ...
    removeChild() {
      for (const child of this.childNotes) {
        if (child) {
          child.removeChild(); // recursively remove children
    ...
    deleteNote() {
      ...
      if (this.parent) {
        // If it has a parent, remove itself from the parent's childNotes
        const index = this.parent.childNotes.indexOf(this);
        if (index !== -1) {
          this.parent.childNotes.splice(index, 1);
        }
      } else {
        // If it's the main note, reset the mainNote variable
        mainNote = null;
      }
      this.removeChild(); // Remove all children recursively
  ...
  function addNote() {
    ...
    if (!mainNote) {
      ...
      mainNote = newNote; // Set the new note as the mainNote
    }
